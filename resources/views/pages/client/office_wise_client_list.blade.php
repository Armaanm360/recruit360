                    ...
                    <div class="card-body">
                        <table id="myTable" class="table display dataTable table-hover" style="width:100%">
                            ...
                            <tbody>
                                ...
                                @php
                                    $photo = '';
                                    $photodetails = App\Models\Clients\Client::clientdetails($client->manpower_client_id);
                                    if($photodetails)
                                    {
                                        $photo = $photodetails->client_picture;
                                    }
                                    $entry = '';
                                    $entryID = App\Models\Clients\Client::clientdetails($client->manpower_client_id);
                                    if($entryID)
                                    {
                                        $entry = $entryID->client_entry_id;
                                    }
                                    $name = '';
                                    $clientname = App\Models\Clients\Client::clientdetails($client->manpower_client_id);
                                    if($clientname)
                                    {
                                        $name = $clientname->client_name;
                                    }
                                    $phone = '';
                                    $clientphone = App\Models\Clients\Client::clientdetails($client->manpower_client_id);
                                    if($clientphone)
                                    {
                                        $phone = $clientphone->client_phone;
                                    }
                                    $email = '';
                                    $clientemail = App\Models\Clients\Client::clientdetails($client->manpower_client_id);
                                    if($clientemail)
                                    {
                                ...
                                @endphp
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{url('view/client')}}/{{$client->manpower_client_id}}"
